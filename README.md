...
- **Answer** — the call is a recursive text search for a code identifier (`grep -r name`, `rg name`, `git grep name`, or the harness's native Grep tool over source trees; declaration hunts like `grep -r "function renderStringAsData"` count too — that is an identifier lookup wearing a two-word coat). The hook runs `find_references --symbol-name <name>` itself (tsconfig discovered from the searched path) and exits 2 with the actual definition and reference list in the message — strictly better data than the grep would have produced, delivered in the same turn. If the lookup cannot answer — no tsconfig above the searched path, the name is not a project symbol (e.g. a library method), the child errors or exceeds its time budget (`TS_SURGEON_ANSWER_TIMEOUT_MS`, default 45s) — **the search is allowed through**: fail-open on reads, so a legitimate grep is never stranded. An ambiguous name (several distinct declarations) is answered with the candidate list, since a text search would have conflated them anyway.
